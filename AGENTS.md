...
- Treat privacy leaks as high-severity issues. Before every commit or push, inspect staged files for private domains, API keys, server aliases, deployment paths, local config, runtime data, and dependency folders.
- Never use broad `git add -A` or `git add .` for privacy fixes or history rebuilds. Add an explicit allowlist of repository files, then verify the staged file list.
- Keep real domains, API keys, server hostnames, private network names, deployment paths, and personal account identifiers out of committed docs, examples, tests, and viewer defaults.
- Real local viewer settings must live only in ignored `viewer/config.local.js`.
